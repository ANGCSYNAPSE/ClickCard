...
import { Gift, Link2, Users, Sparkles, ChevronRight, Star, Trophy, Award } from "lucide-react";
...
function CardOne() {
  return (
    ...
    >
      <div className="rounded-3xl bg-white p-7 shadow-[0_30px_80px_-20px_rgba(124,58,237,0.35)] ring-1 ring-black/5">
        ...
        <div className="relative mt-5 grid h-44 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br from-brand-100 via-mist to-candy-cyan/20">
          {/* floating card */}
          <motion.div
        ...
        <div className="mt-5 space-y-3">
          {[
            ...
            <div key={i} className="flex items-start gap-3">
              ...
              <p className="text-sm font-medium leading-snug text-ink/75 pt-1">{s.t}</p>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-3xl bg-white p-7 shadow-[0_30px_80px_-20px_rgba(236,72,153,0.35)] ring-1 ring-black/5">
        ...
        <div className="flex flex-col items-center pt-4">
          ...
          >
            ...
            <motion.div
              ...
              transition={{ duration: 2, repeat: Infinity }}
              className="absolute -top-1 -right-1 grid h-7 w-7 place-items-center rounded-full bg-brand-gradient text-white shadow-md"
            >
        ...
        <div className="mt-4 space-y-2.5">
          {[
            ...
            <div key={i} className="flex items-center gap-3 rounded-xl bg-mist/70 p-2.5">
              <span className={`grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br ${r.color} text-white`}>
                <Star className="h-4 w-4" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-ink">{r.name}</p>
                <p className="text-xs text-ink/55">Referral bonus · {r.date}</p>
              </div>
              <span className="rounded-full bg-candy-yellow/30 px-2.5 py-1 text-xs font-bold text-candy-orange">
                {r.value} xp
              </span>
            </div>
          ))}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          >
            ...
            <h2 className="mt-5 font-display text-4xl font-extrabold leading-[1.1] text-ink sm:text-5xl md:text-6xl">
              Earn bonuses{" "}
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              <motion.a
                ...
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 rounded-2xl bg-brand-gradient px-6 py-3.5 text-sm font-bold text-white shadow-glow"
              >
                <Sparkles className="h-4 w-4" /> Get your referral code
              </motion.a>
              <a
                href="#features"
                className="inline-flex items-center gap-1.5 rounded-2xl bg-white px-5 py-3.5 text-sm font-bold text-ink shadow-soft ring-1 ring-black/5 hover:ring-black/10"
              >
                Learn how it works
              </a>
            </div>
          ...
          {/* Right: two cards (no phone frames) */}
          <div className="relative flex items-center justify-center gap-4 sm:gap-6">
            {/* Decorative glow behind cards */}
            <div className="absolute inset-0 -z-10 mx-auto h-80 w-80 rounded-full bg-brand-gradient opacity-20 blur-3xl" />
